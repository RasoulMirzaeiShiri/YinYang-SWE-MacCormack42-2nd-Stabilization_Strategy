program SWs_CMC42_Yin_Yang_GSP04_2nd
!******************************************************************************************************
...
! and the second stabilization strategy.
!
...
!******************************************************************************************************
implicit none
integer,parameter::nx=256,ny=(nx/2)+1,kx=512,ky=(kx/2)+1,n_epsilon_x=nx/8,n_epsilon_y=(ny-1)/16&
...
!********************************************************************************************************************
call derivative8(u1,v1,u1e,v1e,dlanda,dphi,nx,ny,mx,my,kkx,kky,dux,dvx,duy,dvy,dunx,dvnx,duny&
...
call derivative6(hene,u1,v1,hee1,u1e,v1e,dlanda,dphi,nx,ny,mx,my,kkx,kky,dux,dvx,duy,dvy,dhx,dhy,dunx,dvnx,duny&
,dvny,dhnx,dhny,duex,dvex,duey,dvey,dhex,dhey,int_method,n_epsilon_x,n_epsilon_y)

call RK_init_swe(u1n,v1n,hen1,dunx,duny,dvnx,dvny,dhnx,dhny,mx,my,a2,hun1,hvn1,hhn1,hhun1,hhvn1,hhhn1,pn,fon,&
delt,u1n,v1n,hen1)


!Yang component

call RK_init_swe(u1e,v1e,hee1,duex,duey,dvex,dvey,dhex,dhey,mx,my,a2,hue1,hve1,hhe1,hhue1,hhve1,hhhe1,pe,foe&
,delt,u1e,v1e,hee1)
call merging_vector_Yin_Yang(hhhn1,hhhe1,hhh,hhun1,hhue1,hhu,hhvn1,hhve1,hhv,nx,ny,mx,my,n_epsilon_x,n_epsilon_y&
,dphi,dlanda,int_method)
...
!********************************************************************************************************************
 ...
 !Yin component
call derivative7(hhh,hhu,hhv,hhhe1,hhue1,hhve1,dlanda,dphi,nx,ny,mx,my,kkx,kky,dux,dvx,duy,dvy,dhx,dhy,dunx,dvnx,duny&
,dvny,dhnx,dhny,duex,dvex,duey,dvey,dhex,dhey,int_method,n_epsilon_x,n_epsilon_y)

call RK_init_swe(u1n,v1n,hen1,dunx,duny,dvnx,dvny,dhnx,dhny,mx,my,a3,hun2,hvn2,hhn2,hhun2,hhvn2,hhhn2,pn,fon,delt&
,hhun1,hhvn1,hhhn1)
...
!********************************************************************************************************************
 ...
 !Yin component
 call derivative6(hhh,hhu,hhv,hhhe2,hhue2,hhve2,dlanda,dphi,nx,ny,mx,my,kkx,kky,dux,dvx,duy,dvy,dhx,dhy,dunx,dvnx,duny&
,dvny,dhnx,dhny,duex,dvex,duey,dvey,dhex,dhey,int_method,n_epsilon_x,n_epsilon_y)

call RK_init_swe(u1n,v1n,hen1,dunx,duny,dvnx,dvny,dhnx,dhny,mx,my,a4,hun3,hvn3,hhn3,hhun3,hhvn3,hhhn3,pn,fon,delt&
,hhun2,hhvn2,hhhn2)

!Yang component

call RK_init_swe(u1e,v1e,hee1,duex,duey,dvex,dvey,dhex,dhey,mx,my,a4,hue3,hve3,hhe3,hhue3,hhve3,hhhe3,pe,foe&
,delt,hhue2,hhve2,hhhe2)
...
!********************************************************************************************************************
 ...
 !Yin component
call derivative7(hhh,hhu,hhv,hhhe3,hhue3,hhve3,dlanda,dphi,nx,ny,mx,my,kkx,kky,dux,dvx,duy,dvy,dhx,dhy,dunx,dvnx,duny&
,dvny,dhnx,dhny,duex,dvex,duey,dvey,dhex,dhey,int_method,n_epsilon_x,n_epsilon_y)


call RK_init_swe(u1n,v1n,hen1,dunx,duny,dvnx,dvny,dhnx,dhny,mx,my,a4,hun4,hvn4,hhn4,hhun4,hhvn4,hhhn4,pn,fon&
,delt,hhun3,hhvn3,hhhn3)
...
call RK_init_swe(u1e,v1e,hee1,duex,duey,dvex,dvey,dhex,dhey,mx,my,a4,hue4,hve4,hhe4,hhue4,hhve4,hhhe4,pe,foe,delt&
,hhue3,hhve3,hhhe3)
...
call derivative8(u2,v2,u2e,v2e,dlanda,dphi,nx,ny,mx,my,kkx,kky,dux,dvx,duy,dvy,dunx,dvnx,duny&
,dvny,duex,dvex,duey,dvey,int_method,n_epsilon_x,n_epsilon_y)
...
end program SWs_CMC42_Yin_Yang_GSP04_2nd
!**************************************************************************
!**************************************************************************
!**************************************************************************
!**************************************************************************
...
sum
!open(131,file=' zonal speed RK4.plt')
!open(141,file='hcapit.plt')
!open(151,file='hprime.plt')
!write(131,*)'zone'
!write(141,*)'zone'
!write(151,*)'zone            i=',nx,'   j=',ny
...
!write(131,*)uphiin(j),phiin(j)*180.0d0/pi
enddo
do i=1,ny
sum=0.0d0
do j=1,i
...
!write(141,*)hcapitin(i),phiin(i)*180.0d0/pi
enddo
...
!write(151,*)landain(i)*180.0d0/pi,phiin(j)*180.0d0/pi,hprimein(i,j)
enddo
enddo
!close(131)
!close(141)
...
!**************************************************************************
!**************************************************************************
subroutine  dbx42(e,dbx,nx,ny,dx,db,eb)
integer::i,j,nx,ny
real*8::e(nx,ny),dbx(nx,ny),a,dx,db(ny),eb(ny)
a=0.5d0-0.5d0/dsqrt(3.0d0)
do j=1,ny
dbx(1,j)=(((e(1,j)-eb(j))/dx)-(a*db(j)))/(1.0d0-a)
do i=2,nx
dbx(i,j)=(((e(i,j)-e(i-1,j))/dx)-(a*dbx(i-1,j)))/(1.0d0-a)
enddo
enddo
...
!**************************************************************************
!**************************************************************************
subroutine  dfx42(e,dfx,nx,ny,dx,df,ef)
integer::i,j,nx,ny
real*8::e(nx,ny),dfx(nx,ny),a,dx,df(ny),ef(ny)
a=0.5d0-0.5d0/dsqrt(3.0d0)
do j=1,ny
dfx(nx,j)=(((ef(j)-e(nx,j))/dx)-(a*df(j)))/(1.0d0-a)
do i=nx-1,1,-1
dfx(i,j)=(((e(i+1,j)-e(i,j))/dx)-(a*dfx(i+1,j)))/(1.0d0-a)
enddo
enddo
...
!**************************************************************************
!**************************************************************************
subroutine  dfy42(e,dfy,nx,ny,dy,df,ef)
integer::i,j,nx,ny
real*8::e(nx,ny),dfy(nx,ny),a,dy,df(nx),ef(nx)
a=0.5d0-0.5d0/dsqrt(3.0d0)
do i=1,nx
dfy(i,ny)=(((ef(i)-e(i,ny))/dy)-(a*df(i)))/(1.0d0-a)
do j=ny-1,1,-1
dfy(i,j)=(((e(i,j+1)-e(i,j))/dy)-(a*dfy(i,j+1)))/(1.0d0-a)
enddo
enddo
...
!**************************************************************************
!**************************************************************************
subroutine  dby42(e,dby,nx,ny,dy,db,eb)
integer::i,j,nx,ny
real*8::e(nx,ny),dby(nx,ny),a,dy,db(nx),eb(nx)
a=0.5d0-0.5d0/dsqrt(3.0d0)
do i=1,nx
dby(i,1)=(((e(i,1)-eb(i))/dy)-(a*db(i)))/(1.0d0-a)
...
!**************************************************************************
!**************************************************************************
subroutine  derivative6(hene,u,v,hee,ue,ve,dlanda,dphi,nx,ny,mx,my,kkx,kky,dux,dvx,duy,dvy,dhx,dhy,dunx,dvnx,duny&
,dvny,dhnx,dhny,duex,dvex,duey,dvey,dhex,dhey,int_method,n_epsilon_x,n_epsilon_y)
implicit none
...
,top_x_limit,top_y_limit,u1(kkx,kky),umex,umey,vmex,vmey,ue(mx,my),ve(mx,my),vn,un,hee(mx,my)&
,du_b(mx),u_b(mx),dv_b(mx),v_b(mx),dh_b(mx),h_b(mx),hm
...
call select_int_method_glob(a10,a11,u,nx,ny,un,dlanda,dphi,int_method)
call select_int_method_glob(a10,a11,v,nx,ny,vn,dlanda,dphi,int_method)
call Trans_vector_Deriv_Yin_to_Yang(a10,a11,a12,a13,un,vn,umnx,vmnx,umny,vmny,umex,vmex,umey,vmey)
du_b(i)=umey
dv_b(i)=vmey
call yinvector_to_yangvector(a10,a11,a12,a13,vn,un,v_b(i),u_b(i))
end do
!***************************************************************************
!pole problem solution 2
!***************************************************************************

call dfx42p(ue,duex,mx,my,dlanda)
call dfx42p(ve,dvex,mx,my,dlanda)
call dfx42p(hee,dhex,mx,my,dlanda)

call dby42(ue,duey,mx,my,dphi,du_b,u_b)
call dby42(ve,dvey,mx,my,dphi,dv_b,v_b)
call dby42(hee,dhey,mx,my,dphi,dh_b,h_b)
!***************************************************************************
return
end subroutine
!**************************************************************************
...
!**************************************************************************
!**************************************************************************
subroutine  derivative7(hene,u,v,hee,ue,ve,dlanda,dphi,nx,ny,mx,my,kkx,kky,dux,dvx,duy,dvy,dhx,dhy,dunx,dvnx,duny&
,dvny,dhnx,dhny,duex,dvex,duey,dvey,dhex,dhey,int_method,n_epsilon_x,n_epsilon_y)
implicit none
integer::i,j,nx,ny,mx,my,kkx,kky,knex,kney,n_epsilon_x,n_epsilon_y
real*8::hene(nx,ny),u(nx,ny),v(nx,ny),dux(nx,ny),dvx(nx,ny),duy(nx,ny),dvy(nx,ny),dhx(nx,ny),dhy(nx,ny)&
...
,duex(mx,my),dvex(mx,my),duey(mx,my),dvey(mx,my),dhex(mx,my),dhey(mx,my)&
,a10,a11,a12,a13,hmnx,hmny,hmex,hmey,umnx,umny,vmnx,vmny,pi,dlanda,dphi,dccy(kkx,kky)&
,top_x_limit,top_y_limit,u1(kkx,kky),umex,umey,vmex,vmey,ue(mx,my),ve(mx,my),un,vn,hee(mx,my)&
,du_f(mx),u_f(mx),dv_f(mx),v_f(mx),dh_f(mx),h_f(mx),hm
character(5)::int_method
kney=(ny-my)/2
knex=(nx-mx)/2
pi=2.0d0*dasin(1.0d0)
...
du_f(i)=umey
dv_f(i)=vmey
call yinvector_to_yangvector(a10,a11,a12,a13,vn,un,v_f(i),u_f(i))
enddo
!***************************************************************************
!pole problem solution 2
!***************************************************************************
call dbx42p(ue,duex,mx,my,dlanda)
call dbx42p(ve,dvex,mx,my,dlanda)
call dbx42p(hee,dhex,mx,my,dlanda)

call dfy42(ue,duey,mx,my,dphi,du_f,u_f)
call dfy42(ve,dvey,mx,my,dphi,dv_f,v_f)
call dfy42(hee,dhey,mx,my,dphi,dh_f,h_f)
!***************************************************************************
return
end subroutine
!**************************************************************************
...
!**************************************************************************
!**************************************************************************
subroutine decx4(a,d,m,k,c)
integer::m,k,i,j
real*8::a(m,k),e(m),d,c(m,k),x(m),y(m)
...
do i=1,m
if(i==1)then
e(i)=(a(i+1,j)-a(m,j))/(2.0d0*d)
elseif(i==m)then
e(i)=(a(1,j)-a(i-1,j))/(2.0d0*d)
else
e(i)=(a(i+1,j)-a(i-1,j))/(2.0d0*d)
endif
enddo
call ptrida(x,y,x,e,1,m)
do i=1,m
c(i,j)=e(i)
enddo
enddo
...
!**************************************************************************
!**************************************************************************
subroutine decy4(a,d,m,k,c)
integer::m,k,i,j
real*8::a(m,k),c(m,k),d,x(k),y(k),e(k)
...
do j=1,k
if(j==1)then
e(j)=(a(i,j+1)-a(i,k))/(2.0d0*d)
elseif(j==k)then
e(j)=(a(i,1)-a(i,j-1))/(2.0d0*d)
else
e(j)=(a(i,j+1)-a(i,j-1))/(2.0d0*d)
endif
enddo
call ptrida(x,y,x,e,1,k)
do j=1,k
c(i,j)=e(j)
enddo
enddo
...
!**************************************************************************
!**************************************************************************
SUBROUTINE PTRIDA(A,B,C,D,IL,IU)
integer::i,il,iu
real*8::ZERO,A(IU),B(IU),C(IU),AA(IU),BB(IU),CC(IU),AAA(IU),BBB(IU),&
CCC(IU),D(IU),X(IU),F(IU),G(IU)
ZERO=0.0D0
DO I=IL,IU-1
F(I)=D(I)
G(I)=ZERO
...
CCC(I)=C(I)
enddo
CALL SYP(IL,IU-1,AA,BB,CC,F)
CALL SYP(IL,IU-1,AAA,BBB,CCC,G)
X(IU)=(D(IU)-C(IU)*F(1)-A(IU)*F(IU-1))/&
(B(IU)+A(IU)*G(IU-1)+C(IU)*G(1))
DO I=IL,IU-1
X(I)=F(I)+G(I)*X(IU)
enddo
DO I=IL,IU
...
!**************************************************************************
!**************************************************************************
subroutine syp(il,iu,bb,dd,aa,cc)
integer::i,il,iu,j,lp
...
!**************************************************************************
!**************************************************************************
subroutine diag_3_nonper(a,b,c,n,d,xx)
integer::n,i
real*8::a(2:n),c(1:n-1),b(1:n),xx(1:n),d(1:n),bb(1:n),dd(1:n)
do i=1,n
bb(i)=b(i)
dd(i)=d(i)
enddo
...
!**************************************************************************
!**************************************************************************
subroutine  derivative8(u,v,ue,ve,dlanda,dphi,nx,ny,mx,my,kkx,kky,dux,dvx,duy,dvy,dunx,dvnx,duny&
,dvny,duex,dvex,duey,dvey,int_method,n_epsilon_x,n_epsilon_y)
implicit none
integer::i,j,nx,ny,mx,my,kkx,kky,knex,kney,n_epsilon_x,n_epsilon_y
...
,top_x_limit,top_y_limit,u1(kkx,kky),umex,umey,vmex,vmey,ue(mx,my),ve(mx,my),vn,un&
,duy_b(mx),duy_f(mx),dvy_b(mx),dvy_f(mx),u_b(mx),u_f(mx),v_b(mx),v_f(mx)
character(5)::int_method
kney=(ny-my)/2
knex=(nx-mx)/2
pi=2.0d0*dasin(1.0d0)
...
call tuu(u1,u,nx,ny,kkx,kky)
call decy4(u1,dphi,kkx,kky,dccy)

call tdu(duy,dccy,nx,ny,kkx,kky)
call decx4(u,dlanda,nx,ny,dux)

call tuu(u1,v,nx,ny,kkx,kky)
call decy4(u1,dphi,kkx,kky,dccy)

call tdu(dvy,dccy,nx,ny,kkx,kky)
call decx4(v,dlanda,nx,ny,dvx)

!call tu(u1,hene,nx,ny,kkx,kky)
!call decy4(u1,dphi,kkx,kky,dccy)

!call td(dhy,dccy,nx,ny,kkx,kky)
!call decx4(hene,dlanda,nx,ny,dhx)
...
!***************************************************************************
call decx4(ue,dlanda,mx,my,duex)
call decx4(ve,dlanda,mx,my,dvex)
!call decx4(hee,dlanda,mx,my,dhex)
call decy4np(ue,duey,mx,my,dphi,duy_f,duy_b,u_f,u_b)
call decy4np(ve,dvey,mx,my,dphi,dvy_f,dvy_b,v_f,v_b)
!call decy4np(hee,dhey,mx,my,dphi,dhy_f,dhy_b,h_f,h_b)
!***************************************************************************
return
end subroutine
!**************************************************************************
...
!**************************************************************************
!**************************************************************************
subroutine  decx4np(e,dcx,mx,my,dx,df,db,ef,eb)
integer::i,j,mx,my
real*8::e(mx,my),dcx(mx,my),dx,db(my),df(my),eb(my),ef(my),a(2:mx),b(mx),c(mx-1),xx(mx),bb(mx)
do j=1,my
a(mx)=1.0d0/6.0d0
b(1)=2.0d0/3.0d0
b(mx)=2.0d0/3.0d0
c(1)=1.0d0/6.0d0
bb(1)=((e(2,j)-eb(j))/(2.0d0*dx))-(1.0d0/6.0d0)*db(j)
bb(mx)=((ef(j)-e(mx-1,j))/(2.0d0*dx))-(1.0d0/6.0d0)*df(j)
do i=2,mx-1
a(i)=1.0d0/6.0d0
b(i)=2.0d0/3.0d0
c(i)=1.0d0/6.0d0
bb(i)=((e(i+1,j)-e(i-1,j))/(2.0d0*dx))
enddo
call diag_3_nonper(a,b,c,mx,bb,xx)
do i=1,mx
dcx(i,j)=xx(i)
enddo
enddo
...
!**************************************************************************
!**************************************************************************
subroutine  decy4np(e,dcy,mx,my,dy,df,db,ef,eb)
integer::i,j,mx,my
real*8::e(mx,my),dcy(mx,my),dy,db(mx),df(mx),eb(mx),ef(mx),a(2:my),b(my),c(my-1),yy(my),bb(my)
do i=1,mx
a(my)=1.0d0/6.0d0
b(1)=2.0d0/3.0d0
b(my)=2.0d0/3.0d0
c(1)=1.0d0/6.0d0
bb(1)=((e(i,2)-eb(i))/(2.0d0*dy))-(1.0d0/6.0d0)*db(i)
bb(my)=((ef(i)-e(i,my-1))/(2.0d0*dy))-(1.0d0/6.0d0)*df(i)
do j=2,my-1
a(j)=1.0d0/6.0d0
b(j)=2.0d0/3.0d0
c(j)=1.0d0/6.0d0
bb(j)=((e(i,j+1)-e(i,j-1))/(2.0d0*dy))
enddo
call diag_3_nonper(a,b,c,my,bb,yy)
do j=1,my
dcy(i,j)=yy(j)
enddo
enddo
